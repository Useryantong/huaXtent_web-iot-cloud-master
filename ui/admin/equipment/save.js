...
importMiniui(function () {
    $(".addbrand-button").click(function () {
        tools.openWindow("admin/equipment/brandsave.html", "添加品牌", "550", "430", function (e) {
            grid.reload();
        })
    });
    $(".addytpe-button").click(function () {
        tools.openWindow("admin/equipment/typesave.html", "添加型号", "550", "430", function (e) {
            grid.reload();
        })
    });
    mini.parse();
...
function loadData(id) {
    require(["request", "message"], function (request, message) {
        ...
        request.get("equipment/" + id, function (response) {
            loading.hide();
            if (response.status === 200) {
